export default function cube(alg) {
  // 设置颜色
  ...
  refresh.addEventListener("click", () => {
    color.map((value, index, arr) => {
      for (let j = 0; j < 9; j++) {
        colorData[index][j] = value;
      }
    });
  ...
  let algArr = inp.value.replace(/[, ]/g, "").split("");
  console.log(algArr);
  let searchFlag = true;
  algArr.map((value) => {
  ...
  //   if (algArr[1] === "2" || algArr[1] === "'") {
  //     newArr.push(algArr[0] + algArr[1]);
  //     algArr.splice(0, 2);
  //     // console.log(algArr);
  //     // break;
  //   } else {
  ...
  //给每个小方块设置颜色
  aside.map((value, index, arr) => {
    let side = document.querySelectorAll("." + value + " div");
    // console.log(side.length);
    for (let i = 0; i < side.length; i++) {
      side[i].style.backgroundColor = color[index];
    }
  });
  ...
  /*0: ['yellow', 'yellow', 'yellow', 'yellow', 'yellow', 'yellow', 'yellow', 'yellow', 'yellow']
    ...
    3:  ['blue', 'blue', 'blue', 'blue', 'blue', 'blue', 'blue', 'blue', 'blue']
    4:  ['red', 'red', 'red', 'red', 'red', 'red', 'red', 'red', 'red']
    5:  ['orange', 'orange', 'orange', 'orange', 'orange', 'orange', 'orange', 'orange', 'orange'] */
  let colorData = [];
